...
for key, value in opening_room_dict.items():
    ...
    # It is a window
        ...
        room_center = value[0].geometry.center
        opening_room_center = key.geometry.center
        line = LineSegment3D.from_end_points(opening_room_center, room_center)
        for face in value[0].geometry.faces:
            if not intersect_line3d_plane(line, face.plane):
                continue
            else:
                room_normal = face.normal
        for face in key.geometry.faces:
            print(face.normal, room_normal, face.normal.dot(room_normal))
            if face.normal.dot(room_normal) != -1:
                continue
            else:
                apertures.append(Aperture(clean_and_id_string('Aperture'), face))
